...
void Font_cairo::loadFont(const string& file) {
  const string& corrected_file = file;
  auto ffaceEntry = _cairoFtFaces.find(corrected_file);
  auto familyEntry = _families.find(corrected_file);
  if (ffaceEntry != _cairoFtFaces.end() && familyEntry != _families.end()) {
    // already loaded
    _family = familyEntry->second;
    _fface = ffaceEntry->second;
#ifdef HAVE_LOG
    __log << corrected_file << " already loaded, skip\n";
#endif
    return;
  }

  // query font via fontconfig
  const FcChar8* f = (const FcChar8*) corrected_file.c_str();

  // get font family from corrected_file first
  int count;
  FcChar8* family = NULL;
